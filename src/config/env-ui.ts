...
export class EnvUI extends LitElement {
  ...
  render() {
    return html`
      ...
      <d2l-input-text label="File Title" placeholder="Enter file title..."></d2l-input-text>
      ...
      <mam-toast-alerts id="toast"></mam-toast-alerts> <!-- 👈 Include Toast Renderer -->
    `;
  }
  ...
  private _openMamDialog(): void {
    const dialog = this.shadowRoot?.getElementById('mamDialog') as HTMLElement & { openDialog?: () => void };
    dialog?.openDialog?.();
  }
